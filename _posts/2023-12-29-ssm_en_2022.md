...
|:--:|
| Figure X : *Vue d’ensemble du Mega, figure conçue à partir de ma compréhension du papier.<br>Les auteurs remplacent la RELU² du GAU par une fonction de Laplace qui est plus stable.* |
...
$$
 \begin{aligned}
      ...
      \\
      \frac{\partial}{\partial t^{(2)}} x(t^{(1)}, t^{(2)}) &= (x^{(1)}(t^{(1)}, t^{(2)}), \bm{A}^{(2)} x^{(2)}(t^{(1)}, t^{(2)})) + \mathbf{B}^{(2)} u(t^{(1)}, t^{(2)})
      \\
