...
Les travaux menés permettent d’améliorer les résultats du S4 sur le benchmark LRA de plus de 5,5 points :
...
$$.  
Le tout est calculable en $$\mathbf{O}(N+L)$$ opérations et espace.  
...
Les auteurs appliquent leur modèle également sur les jeux de données Speech Commands, sCIFAR et BIDMC Vital Signs et y établissent le nouvel état de l'art.  
...
Notons qu'en plus du LRA, les auteurs du S5 comparent leur modèle sur le *Speech Commands dataset*, le *pendulum regression dataset* ainsi que *sMNIST*, *psMNIST* et *sCIFAR*. L'ensemble des résultats sont disponibles dans l'Appendix du papier qui contient également une étude d'ablation.
<br>
...
Avec le SGConv et le S5, nous pouvons aussi voir les prémisses d'un phénomène qui s'est, comme nous le verrons dans l’article suivant, accentué en 2023. A savoir l’émergence de travaux se focalisant uniquement sur la vue convolutive des SSM (par exemple le Hyena et ses dérivés) ou se focalisant uniquement sur la vue récurrente des SSM (par exemple le Mamba).
<br><br><br>
