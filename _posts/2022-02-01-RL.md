...
2. Générer un nouvel épisode avec la nouvelle politique $$\pi$$ (c'est-à-dire que l'utilisation d'algorithmes comme ε-greedy nous aide à trouver un équilibre entre exploitation et exploration).
...
Avec toutes les connaissances de RL ci-dessus, jetons un coup d'œil sur le fonctionnement d'AlphaGo Zero. Le composant principal est un ConvNet sur la configuration du plateau de jeu (précisément, un ResNet avec batch-normalisation et ReLU). Ce réseau produit deux valeurs :
