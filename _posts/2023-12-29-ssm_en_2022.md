...
Ci-dessous un tableau comparatif des valeurs de $$\mathbf{A}$$, $$\mathbf{B}$$ et $$\mathbf{C}$$ pour chacunes des deux discrétisations dans la vue récurrente, ainsi que l’expression du noyau de convolution dans la vue convolutive :
...
L'initialisation de la matrice d'état $$\mathbf{A}$$ du DSS repose sur une approximation particulière de la matrice HiPPO du S4. Si la matrice du S4 a une interprétation mathématique pour traiter les dépendances à longue portée, l'efficacité de l'approximation diagonale reste théoriquement inexpliquée.  
Avec le S4D, les auteurs introduisent un SSM diagonal combinant le meilleur du calcul et de la paramétrisation de S4 et de l'initialisation de DSS, ce qui donne une méthode extrêmement simple, théoriquement fondée et empiriquement efficace.  
...
Le 1er décembre 2022, GUPTA et al. présentent une suite au DSS avec [*Simplifying and Understanding State Space Models with Diagonal Linear RNNs*](https://arxiv.org/abs/2212.00768) qui introduit le DLR. Ils se débarrassent de l'étape de discrétisation et proposent un modèle basé sur des RNN linéaires diagonaux (DLR) pouvant opérer sur envrion 1M de positions (contrairement aux RNN classiques). Le code de ce modèle est disponible sur [GitHub](https://github.com/ag1988/dlr).
<br><br>
...
Dans ce papier, les auteurs utilisent la formulation des SSM structurels (S4) pour obtenir des instances de réseaux liquides linéaires possèdant les capacités d'approximation du S4 et des LTC (*liquid time-constant*).  
...
Pour en savoir plus sur les réseaux liquides, vous pouvez consultez un papier précédent par les mêmes auteurs : [*Liquid Time-constant Networks*](https://ojs.aaai.org/index.php/AAAI/article/view/16936) (2021).  
...
La vue récurrente du Liquid-S4 est obtenue en discrétisant le sustème avec la règle des trapèzes (forme bilinéaire). On obtient alors :
\begin{align}
...
2. En violet, les poids associés à tous les ordres d'auto-corrélation du signal d'entrée. Il s'agit d'un noyau de corrélation d'entrée supplémentaire, appellé noyau liquide par les auteurs.
...
Testée sur le LRA, cette approche établit apparait comme la meilleure. Seul Mega publié la semaine précédant le papier et donc non visible ici fait mieux :

| ![image](https://raw.githubusercontent.com/lbourdois/blog/master/assets/images/SSM_2022/LRA_Liquid_S4.png) |
|:--:|
| Figure 17 : *Les résultats du Liquid-S4 sur le benchmark LRA* |

<br>

Les auteurs appliquent leur modèle également sur les jeux de données Speech Commands, sCIFAR et [BIDMC Vital Signs](https://ieeexplore.ieee.org/document/7748483) de PIMENTEL et al., et y établissent le nouvel état de l'art.  
<br>
<u>Pour aller plus loin</u>  
...
Techniquement, le [*Simplified State Space Layers for Sequence Modeling*](https://arxiv.org/abs/2208.04933) de SMITH, WARRINGTON et LINDERMAN introduisant le modèle S5 a été dévoilé le 9 août 2022, donc avant le Mega et le Liquid-S5. Cependant j'aborde ce papier après ces deux-là car le 6 octobre 2022, les auteurs du S5 ont procédé à une actualisation où dans leur V2, ils améliorent leur modèle de plus de 5 points sur le LRA par rapport à la V1. De plus, ils proposent une comparaison portant sur l'ensemble des SSM sortis en 2022 (à l'exception des deux suivants que je traite ci-dessous). Cela me paraissait donc plus interessant d'aborder le S5 à partir de la V2.  
...
| ![image](https://raw.githubusercontent.com/lbourdois/blog/master/assets/images/SSM_2022/S5vsS4.png) |
|:--:|
| Figure 18 : *Le comportement interne du S4 vs celui du S5* |

<br>

La dimension latente réduite du système MIMO permet l'utilisation de l'algorithme du *parallel scan* qui simplifie les calculs nécessaires pour appliquer la couche S5 en tant que transformation séquence à séquence. Le modèle résultant perd la vue convolutive du SSM pour se focaliser uniquement sur la vue récurrente (obtenue par discrétisation ZOH). Le parti pris des auteurs est donc d'opérer sur le domaine temporel plutôt que le domaine fréquentiel. Ils utilisent une approximation diagonale de la matrice HiPPO qui leur permettant d'avoir une initialisation et une paramétrisation efficaces adaptées à leur système MIMO.

| ![image](https://raw.githubusercontent.com/lbourdois/blog/master/assets/images/SSM_2022/S5.png) |
|:--:|
| Figure 19 : *Comparaison complète du fonctionnement du S4 vs celui du S5* |

<br>

L'utilisation du *parallel scan* étant un composant repris dans d'autres SSM par la suite (le Mamba notamment), détaillons un peu son fonctionnent dans le cadre du S5 afin de se familiariser avec cet algortihme dès cet article. Pour cela le plus simple est de reprendre l'exemple donnait dans l'appendix H du papier où les auteurs l'applique sur une séquence de longeur $$L = 4$$.  

Pour calculer un *parallel scan* nous avons besoin de deux choses :  
...
Pour obtenir les états $$x_i$$, nous devrions alors prendre le deuxième élement de chaque tuple $$s_i$$.  
...
A nouveau, pour obtenir les états $$x_i$$, nous devrions alors prendre le deuxième élement de chaque tuple $$s_i$$.  
...
|:--:|
| Figure 20 : *Fonctionnement du parallel scan d'une manière générale* |
...
<br>
Notons qu'en plus du LRA, les auteurs du S5 comparent leur modèle sur le *Speech Commands*, le *pendulum regression dataset* ainsi que *sMNIST*, *psMNIST* et *sCIFAR*. L'ensemble des résultats sont disponibles dans l'Appendix du papier qui contient également une étude d'ablation.
<br>
<u>Pour aller plus loin</u>  
...
Ils identifient deux principes critiques dont bénéficie S4 et qui sont suffisants pour constituer un modèle convolutif global efficace :  
...
| ![image](https://raw.githubusercontent.com/lbourdois/blog/master/assets/images/SSM_2022/SGConv.png) |
|:--:|
...
En effet, en regardant ce tableau, on peut effectivement constater que le SGConv fait mieux que les deux versions du S4. Néanmoins, il est curieux que les auteurs n’intègrent pas le Mega, le Liquid-S4 ou encore le S5 dans leur comparaison qui pourtant obtiennent de meilleurs résultats et utilisent un noyau de convolution qui est une somme de fonctions exponentielles décroissantes.  
De plus, alors que tous les modèles qui se sont comparés sur le LRA traitent les données comme des séquences 1D, le SGConv intègre implicitement un biais inductif 2D pour les tâches d'image, y compris PathX ce qui est questionnable.  

Au final, le SGConv semble donc avoir des performances similaires aux variantes de SSM les plus récentes alors mais tout en perdant la vue récurrente du S4.    
Ce papier apparait néanmoins comme le premier se focalisant uniquement sur la vue convolutive d'un SSM.
<br>
<u>Pour aller plus loin</u>  
...
Techniquement, deux autres papiers « théoriques » ont été publié en 2022. Le [*Pretraining Without Attention*](https://arxiv.org/abs/2212.10544) de WANG et al. qui présente le BiGS et le [*Hungry Hungry Hippos: Towards Language Modeling with State Space Models*](https://arxiv.org/abs/2212.14052) de FU, DAO et al. introduisant le H3.  
Du fait de leur publication tardive (respectivement le le 20 décembre et le 28 décembre 2022) et que la communication de ses papiers ayant été effectué en 2023 après les V2 de chacun de ses papiers, je traiterais ces modèles dans le prochain article de la série sur les SSM.  
<br><br>
...
| ![image](https://raw.githubusercontent.com/lbourdois/blog/master/assets/images/SSM_2022/CCNN.png) |
|:--:|
...
Les diapositives d'un présentation du papier est disponible [ici](https://app.slidebean.com/sbp/gk5j826nq7/CCNN#1)
<br><br>
...
L'intérêt principal du S4ND étant qu'il peut fonctionner avec différentes résolutions via différents taux d'échantillonage. Les auteurs mettent en avant cette caractéristique à travers deux expériences :  
...
<u>Pour aller plus loin</u>  
L'implémentation officielle est disponible sur [GitHub](https://github.com/AI4HealthUOL/SSSD).  
Une information complètement inutile mais qui m'a fait rire : les auteurs ont appelé leur fichier TeX [Darude S4NDstorm](https://www.youtube.com/watch?v=y6120QOlsfU).
<br><br><br>


# <span style="color: #FF0000"> **Conclusion** </span>
Nous avons donc fait une revue des différents modèles de SSM parus en 2022. 
Il s'agit d'une année où les travaux se sont principalement portés sur une amélioration/simplification du S4 via diverses approches (diagonalisation, *gating*, LTC, etc.). Lors de cette année 2022, nous avons également pu voir de premières applications des SSM.  
Avec le SGConv et le S5, nous pouvons aussi voir les prémices d'un phénomène qui s'est, comme nous le verrons dans l’article suivant, accentué en 2023. A savoir l’émergence de travaux se focalisant uniquement sur la vue convolutive des SSM (par exemple le Hyena et ses dérivés) ou se focalisant uniquement sur la vue récurrente des SSM (par exemple le Mamba).
<br><br><br>
